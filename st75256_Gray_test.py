...
#iic=I2C(1,sda=Pin(33,Pin.PULL_UP),scl=Pin(32,Pin.PULL_UP), freq=600000)
spi=SPI(1, 20000000, sck=Pin(32), mosi=Pin(33), miso=Pin(34))
#from ST75256_Gray import ST75256_I2C
from ST75256_Gray import ST75256_SPI
...
#rst(1)
#time.sleep(0.02)
#oled = ST75256_I2C(256, 96, iic, res=Pin(25), rot=2)
oled = ST75256_SPI(256, 96, spi, res=Pin(25), cs=Pin(26), dc=Pin(27), rot=2)
oled.contrast(260)#可用取值范围0-511，实际有用的区间大约在245-280区间，过小是全白，过大就是全黑
...
if 0:
    ...
    for i in range(4):
        ...
        oled.text("MicroPython中文显示",0,i*20,i)
    oled.show()
    time.sleep(3)
...
if 0:#改成0则跳过字体演示
    ...
    oled.text("MicroPython 中文显示",0,0,3)
    oled.font_set(0x22,0,1,0)
    oled.text("MicroPython 中文显示",0,18,3)
    oled.font_set(0x32,0,1,0)
    oled.text("MicroPython 中文显示",0,96,3)
    oled.font_set(0x42,0,1,0)
    oled.text("MicroPython 中文显示",0,154,3)
    oled.show()
    time.sleep(3)
    oled.fill(0)
    ...
    oled.text("MicroPython 中文显示",0,26,3)
    oled.show()
    time.sleep(3)
    oled.fill(0)
    ...
    oled.text("MicroPython 中文显示",0,36,3)
    oled.show()
    time.sleep(3)
    oled.fill(0)
...
if 0:#字体背景，写0跳过
    ...
    oled.fill(0)
    oled.show_bmp("logo96gray.bmp",0,0)
...
if 1:#速度测试
    ...
    for count in range(100):
        ...
        oled.font_set(0x12,0,1,0)
        oled.text("MicroPython中文显示=%d"%count,0,16,2)
        oled.font_set(0x13,0,1,0)
        oled.text("Mpy中文显示=%d"%count,0,36,3)
        oled.font_set(0x14,0,1,0)
...
if 1:#图片显示演示
    ...
    for count in range(80):
        oled.fill(0)
        oled.show_bmp("logo96gray.bmp",1,0,count)
        oled.show()
    oled.save_bmp("logo96-1.bmp")
